   ...
   * .line({x: 10, y: 20}, {x: 30, y: 40});
	...
	line(point_a, point_b, options = {}) {
		...
		if (options.line_fix){
			...
			let shift = 0;
			if (((dx > 0 && dy > 0 && Math.abs(deg) <= correction_angle) || (dx < 0 && dy > 0 && Math.abs(deg - 180) <= correction_angle))) {
		...
		if (dx < 0 && dy < 0) { // fix: bottom-right to top-left
			x = center_x + (length / 2) * Math.cos(angle);
			y = center_y + (length / 2) * Math.sin(angle);
			deg += 180; // mirror
		} else if (dx > 0 && dy < 0) { // fix: top-right to bottom-left
				x = center_x - (length / 2) * Math.cos(angle);
				y = center_y + (length / 2) * Math.sin(angle);
				deg += 180;
		} else if (dx < 0 && dy > 0) { // fix: bottom-left to top-right
				x = center_x + (length / 2) * Math.cos(angle);
				y = center_y - (length / 2) * Math.sin(angle);
				deg += 180;
		} else if (is_diagonal) { // shift pos based on the angle
				x = center_x - (length / 2) * Math.cos(angle);
				y = center_y - (length / 2) * Math.sin(angle);
		} else {
				if (Math.abs(dx) > Math.abs(dy)) { // is_horizontal
						x = center_x - length / 2;
   ...
   * .circle(50, 50);
	...
	circle(center_x, center_y, options = {}) {
		...
		const circle = this.createWidget(widget.CIRCLE, { 
			...
			...this.default(center_x - options.radius, center_y - options.radius, options.radius * 2, options.radius * 2), 
			...options 
		});
   ...
   */
	text(text, options = {}) {
		...
		const text_widget = this.createWidget(widget.TEXT, { 
			text: text, ...this.default(options.x, options.y, options.w, options.h), ...options 
		});
